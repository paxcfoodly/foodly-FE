...
const COLUMNS: DataGridColumn<InspectResult>[] = [
  ...
  },
  {
    ...
    width: 140,
    render: (val: unknown) => {
      const wo = val as { wo_no?: string } | null;
      return wo?.wo_no ?? '-';
    },
  },
  {
  ...
  },
  {
    ...
    width: 140,
    render: (val: unknown) => {
      const process = val as { process_nm?: string } | null;
      return process?.process_nm ?? '-';
    },
  },
  {
    title: '판정',
...
const EXCEL_COLUMNS = [
  ...
  { header: '작업지시', key: 'wo_no', width: 18 },
...
export default function ProcessInspectionPage() {
  ...
  const handleSearch = useCallback((values: Record<string, unknown>) => {
    ...
    if (values.inspect_no) params.inspect_no = values.inspect_no;
  ...
  const getExcelData = useCallback(async () => {
    const res = await apiClient.get('/v1/inspect-results', {
      params: { inspect_type: 'PROCESS', limit: 10000, ...searchParams },
    });
    const rows: InspectResult[] = res.data?.data ?? [];
    return rows.map((r) => ({
      inspect_no: r.inspect_no,
      wo_no: r.work_order?.wo_no ?? '',
      item_nm: r.item?.item_nm ?? '',
      lot_no: r.lot_no,
      process_nm: r.process?.process_nm ?? '',
