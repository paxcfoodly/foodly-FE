  ...
  return (
    <div>
      ...
      <FormModal<ResultFormValues> open={modalOpen} onClose={() => { setModalOpen(false); setEditItem(null); }} onSubmit={handleSubmit} mode={modalMode} initialValues={modalInitialValues} title={modalMode === 'create' ? '실적 등록' : '실적 수정'} width={560}>
        {(form, mode) => (
          <Section title="실적 정보">
            <Row label="작업지시" required><Select name="wo_id" placeholder="작업지시 선택" required disabled={mode === 'edit'} options={workOrders.map((wo) => ({ label: `${wo.wo_no} — ${wo.item?.item_nm ?? wo.item_cd}`, value: String(wo.wo_id) }))} defaultValue={form.getFieldsValue().wo_id ? String(form.getFieldsValue().wo_id) : ''} onChange={(e) => form.setFieldsValue({ wo_id: Number(e.target.value) } as any)} /></Row>
            <Row label="설비"><Select name="equip_cd" placeholder="설비 선택" options={[{ label: '선택 안함', value: '' }, ...equipments.map((eq) => ({ label: `${eq.equip_cd} — ${eq.equip_nm}`, value: eq.equip_cd }))]} defaultValue={form.getFieldsValue().equip_cd ?? ''} onChange={(e) => form.setFieldsValue({ equip_cd: e.target.value } as any)} /></Row>
            <Row label="작업자"><Select name="worker_id" placeholder="작업자 선택" options={[{ label: '선택 안함', value: '' }, ...workers.map((wk) => ({ label: wk.worker_nm, value: String(wk.worker_id) }))]} defaultValue={form.getFieldsValue().worker_id ? String(form.getFieldsValue().worker_id) : ''} onChange={(e) => form.setFieldsValue({ worker_id: e.target.value ? Number(e.target.value) : undefined } as any)} /></Row>
            <Row label="양품수량" required><input type="number" name="good_qty" placeholder="양품수량" min={0} step={1} required className="w-full h-9 bg-dark-700 border border-dark-500 rounded-lg px-3 text-sm text-gray-700 focus:outline-none focus:bg-white focus:border-cyan-accent focus:ring-2 focus:ring-cyan-accent/15" defaultValue={form.getFieldsValue().good_qty ?? ''} onChange={(e) => form.setFieldsValue({ good_qty: Number(e.target.value) } as any)} /></Row>
            <Row label="불량수량"><input type="number" name="defect_qty" placeholder="불량수량" min={0} step={1} className="w-full h-9 bg-dark-700 border border-dark-500 rounded-lg px-3 text-sm text-gray-700 focus:outline-none focus:bg-white focus:border-cyan-accent focus:ring-2 focus:ring-cyan-accent/15" defaultValue={form.getFieldsValue().defect_qty ?? ''} onChange={(e) => form.setFieldsValue({ defect_qty: Number(e.target.value) } as any)} /></Row>
            <Row label="작업시작"><input type="datetime-local" name="work_start_dt" className="w-full h-9 bg-dark-700 border border-dark-500 rounded-lg px-3 text-sm text-gray-700 focus:outline-none focus:bg-white focus:border-cyan-accent focus:ring-2 focus:ring-cyan-accent/15" defaultValue={form.getFieldsValue().work_start_dt as string ?? ''} onChange={(e) => form.setFieldsValue({ work_start_dt: e.target.value } as any)} /></Row>
            <Row label="작업종료"><input type="datetime-local" name="work_end_dt" className="w-full h-9 bg-dark-700 border border-dark-500 rounded-lg px-3 text-sm text-gray-700 focus:outline-none focus:bg-white focus:border-cyan-accent focus:ring-2 focus:ring-cyan-accent/15" defaultValue={form.getFieldsValue().work_end_dt as string ?? ''} onChange={(e) => form.setFieldsValue({ work_end_dt: e.target.value } as any)} /></Row>
            {mode === 'create' && (
              <Row label="LOT 자동생성"><label className="flex items-center gap-2"><input type="checkbox" name="auto_lot" defaultChecked={form.getFieldsValue().auto_lot as boolean ?? true} onChange={(e) => form.setFieldsValue({ auto_lot: e.target.checked } as any)} className="accent-cyan-accent" /><span className="text-sm text-gray-700">자동 LOT 생성</span></label></Row>
            )}
          </Section>
        )}
      </FormModal>
